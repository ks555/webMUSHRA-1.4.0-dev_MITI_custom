...
function LikertScale(_responseConfig, _prefix, _initDisabled, _callback, _width='100%', _height='100%') {
...
LikertScale.prototype.render = function (_parent) {
  ...
  for (i = 0; i < this.responseConfig.length; ++i) {
    ...
    if (responseValueConfig.img) {
      img = "<img id='"+this.prefix+"_response_img_"+i+"' src='"+responseValueConfig.img+"' width='"+this.width+"' height='"+this.height+"'/><br/>";
    }
    var element = $("<input type='radio' data-mini='false' value='"+responseValueConfig.value+"' name='"+this.prefix+"_response' id='"+this.prefix+"_response_"+i+"'/> \
      <label for='"+this.prefix+"_response_"+i+"' style=font-size:60%><center>"+img+""+responseValueConfig.label+"</center></label> \
    ");
